...
import { 
    selectUserPins, selectSuggestedPins, selectOtherUsersPins 
...
const mapStateToProps = ({entities, session}, {match: {params}}) => {
    ...
    if (params.boardId) {
        ...
        if ( parseInt(params.userId) === session.currentUserId ) 
            createOption = true;
    } else if (params.pinId){
        pins = selectSuggestedPins(
            entities.pins, session.currentUserId, parseInt(params.pinId)
        )
    } else {
        pins = selectOtherUsersPins
            (entities.pins, parseInt(session.currentUserId)
        );
        mainFeed = true;
    }
